...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // AUTOPLAY HARD EXECUTE
    setTimeout(function () {
        console.log('Checking for video autoplay after delay...');
        var video = document.querySelector('video[autoplay]');
        if (video) {
            console.log('Video element found:', video);
            video.muted = true;
            video.play().catch(function (error) {
                console.warn('Autoplay not activated:', error);
            });
        } else {
            console.log('No video element found with autoplay attribute.');
        }
    }, 1000); 

    // OVERRIDE CUSTOM FORM 7 ADD FUNTIONALITY
    document.addEventListener('wpcf7mailsent', function (event) {
        var formId = event.detail.contactFormId;
        var formElement = document.querySelector('div.wpcf7[data-wpcf7-id="' + formId + '"]'); 
        if (!formElement) {
            console.error('Formulario no encontrado en el DOM.');
            return;
        }

        var parentContainer = formElement.closest('.form-container');
        if (parentContainer) {
            var redirectUrl = parentContainer.getAttribute('data-redirect');
            var squeezePage = parentContainer.getAttribute('data-squeeze');

            if (redirectUrl && redirectUrl !== '0') {
                window.location.href = redirectUrl;
            }
        } else {
            console.warn('No se encontró el contenedor padre con la clase .form-container');
        }
    }, false);
